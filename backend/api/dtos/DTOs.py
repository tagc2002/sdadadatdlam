...
def get_url(req: Request, method: str, query_params: Optional[Dict[str, Any]] = None, **path_kwargs) -> HttpUrl:
...
class ClaimDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Claim, req: Request) -> Self:
        ...
        # model.agreements = [get_url(req, "get_agreement", agreement_id=item.agreementID) for item in sql.agreements]
        # model.citations = [get_url(req, "get_citation", citation_id=item.citationID) for item in sql.citations]
        # model.citations = [get_url(req, "get_complaint", complaint_id=item.complaintID) for item in sql.complaints]
        # model.documentation = DocumentationLinkDTO.from_list(sql.documentationLink, req)
        # model.employees = [get_url(req, "get_employee", employee_id=item.employeeID) for item in sql.employees]
        # model.employers = [get_url(req, "get_employer", employer_id=item.employerID) for item in sql.employers]
        # model.lawyers = [get_url(req, "get_lawyer", lawyer_id=item.lawyerID) for item in sql.lawyers]
        # model.nonagreements = [get_url(req, "get_nonagreement", non_id=item.nonID) for item in sql.nonagreements]
        return model
...
class CitationDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Citation, req: Request) -> Self:
        model = cls.model_validate(sql)
        if sql.agreement:
            model.agreement = get_url(req, "get_agreement", agreement_id=sql.agreement.agreementID)
        model.claim = get_url(req, "get_claim", rec_id=sql.recID)
        model.nonagreement = get_url(req, "get_agreement", rec_id=sql.recID, nonagreement_id=sql.nonagreement.nonID) if sql.nonagreement else None
        model.notifications = get_url(req, "get_notifications", query_params={"citation_id": sql.citationID})
        model.lawyerToEmployee = LawyerToEmployeeDTO.from_list(sql.lawyerToEmployee, req)
        model.lawyerToEmployer = LawyerToEmployerDTO.from_list(sql.lawyerToEmployer, req)
        return model
...
class NotificationDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: SecloNotification, req: Request) -> Self:
        ...
        if sql.employeeLink:
            model.belongsTo = get_url(req, "get_employee", employee_id=sql.employeeLink.employeeID)
        if sql.employerLink:
            model.belongsTo = get_url(req, "get_employer", employer_id=sql.employerLink.employerID)
        return model
...
class DocumentationLinkDTO(MyBaseModel):
    ...
    @classmethod
    def from_sql(cls, sql: DocumentationEmployeeLink | DocumentationEmployerLink | DocumentationLawyerLink | DocumentationAgreementLink | DocumentationNonagreementLink | Homologation | Invoice | Payment | DocumentationObservationLink | DocumentationClaimLink, req: Request) -> Self:
        model = cls.model_validate(sql)
        ...
        if isinstance(sql, DocumentationAgreementLink):
            model.belongsTo=get_url(req, "get_agreement", agreement_id=sql.agreementID)
...
class DocumentationDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Documentation, req: Request) -> Self:
        ...
        doc_links.extend(sql.employeeLink + sql.employerLink + sql.lawyerLink +
                         sql.agreementLink + sql.nonagreementLink + sql.claimLink)
        model = cls.model_validate(sql)
...
class BankAccountDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: BankAccount, req: Request) -> Self:
        model = cls.model_validate(sql)
        if sql.employee:
            model.belongsTo = [get_url(req, "get_employee", employee_id=sql.employee.employeeID)]
        else:
            model.belongsTo = ([get_url(req, "get_lawyer", lawyer_id=entry.lawyerID) for entry in sql.lawyers])
        return model
...
class AddressDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Address, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.belongsTo = [get_url(req, "get_employee", employee_id=employee.employeeID) for employee in sql.employees] + \
                          [get_url(req, "get_employer", employer_id=employer.employerID) for employer in sql.employers]
        return model
...
class BelongsDTO(MyBaseModel):
    ...
    @classmethod
    def from_sql(cls, sql: EmployeeEmailLink | EmployerEmailLink | LawyerEmailLink | BeneficiaryEmailLink | EmployeeAddressLink | EmployerAddressLink | BeneficiaryAddressLink | LawyerDirectoryPhoneLink | LawfirmDirectoryPhoneLink | LawyerDirectoryEmailLink | LawfirmDirectoryEmailLink, req: Request) -> Self:
        model = cls.model_validate(sql)
        if isinstance(sql, EmployeeEmailLink) or isinstance(sql, EmployeeAddressLink):
            model.owner=get_url(req, "get_employee", employee_id=sql.employee.employeeID)
        if isinstance(sql, EmployerEmailLink) or isinstance(sql, EmployerAddressLink):
            model.owner=get_url(req, "get_employer", employer_id=sql.employer.employerID)
...
class EmailDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Email, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.belongsTo = [BelongsDTO.from_sql(x, req) for x in sql.employees + sql.employers + sql.lawyers]
        return model
...
class EmployeeDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Employee, req: Request) -> Self:
        model = cls.model_validate(sql)
        if sql.bankAccount:
            model.bankAccount=get_url(req, "get_employee_bank_account", employee_id=sql.employeeID)
        model.claim=get_url(req, "get_claim", rec_id=sql.recID)
        model.addresses = [get_url(req, "get_employee_address", employee_id=sql.employeeID, address_id=address.addressID) for address in sql.addresses]
        model.emails = [get_url(req, "get_employee_emails", employee_id=sql.employeeID, email_id=email.emailID) for email in sql.emails]
        model.notifications = get_url(req, "get_notifications", query_params={"employee_id":sql.employeeID, "rec_id":sql.recID})
        ...
        if sql.hemiagreement:
            model.hemiagreement = get_url(req, "get_hemiagreement", hemi_id=sql.hemiagreement.hemiID)
        model.relationshipData = [get_url(req, "get_employee_relationship", relationship_id=entry.employeeDataID) for entry in sql.relationshipData]
        return model
...
class EmployerDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Employer, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.claim=get_url(req, "get_claim", rec_id=sql.recID)
        model.addresses = [get_url(req, "get_employer_address", address_id=address.addressID) for address in sql.addresses]
        model.emails = [get_url(req, "get_employer_emails", email_id=email.emailID) for email in sql.emails]
        model.notifications = get_url(req, "get_notifications", query_params={"employer_id":sql.employerID, "rec_id": sql.recID})
        model.documentation = DocumentationLinkDTO.from_list(sql.documentation, req)
        model.lawyers = LawyerToEmployeeDTO.from_list(sql.lawyerLink, req)
        return model
...
class LawyerDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Lawyer, req: Request) -> Self:
        ...
        if sql.bankAccountID:
            model.bankAccount=get_url(req, "get_bank_account", account_id=sql.bankAccountID)
        model.claim=get_url(req, "get_claim", rec_id=sql.recID)
        model.emails = [get_url(req, "get_email", email_id=email.emailID) for email in sql.emails]
        model.documentation = DocumentationLinkDTO.from_list(sql.documentation, req)
...
class HemiagreementDTO(MyBaseModel):
    ...
    installments:       SkipValidation[Optional[List[HttpUrl] | List[PaymentInstallmentDTO]]] = None
    agreement:          SkipValidation[Optional[HttpUrl]] = None
    ...
    def from_sql(cls, sql: Hemiagreement, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.installments = [get_url(req, "get_installments", installment_id=entry.installmentID) for entry in sql.installments]
...
class AgreementDTO(MyBaseModel):
    ...
    hemiagreements: SkipValidation[Optional[List[HemiagreementDTO] | List[HttpUrl]]] = None
    ...
    def from_sql(cls, sql: Agreement, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.hemiagreements = [get_url(req, "get_hemiagreement", hemi_id=hemi.hemiID) for hemi in sql.hemiagreements]
        model.homologations = [get_url(req, "get_homologation", homo_id=homo.homoID) for homo in sql.homologations]
        model.invoices = [get_url(req, "get_invoice", invoice_id=invoice.invoiceID) for invoice in sql.invoices]
        model.payments = [get_url(req, "get_payment", payment_id=payment.paymentID) for payment in sql.payments]
        model.observations = [get_url(req, "get_observation", obs_id=obs.obsID) for obs in sql.observations]
        model.complaints = [get_url(req, "get_complaints", complaint_id=complaint.complaintID) for complaint in sql.complaintLink]
        model.documentation = [get_url(req, "get_document", doc_id=doc.docID) for doc in sql.documentationLink]
        model.extension = [get_url(req, "get_employer", employer_id=ext.employerID) for ext in sql.extension]
        model.desist = [get_url(req, "get_employer", employer_id=ext.employerID) for ext in sql.desist]
        model.claim = get_url(req, "get_claim", rec_id=sql.recID)
        if sql.citationID:
            model.citation = get_url(req, "get_citation", citation_id=sql.citationID)
        return model
...
class HomologationDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Homologation, req: Request) -> Self:
        ...
        if sql.complaintLink:
            model.complaints = [get_url(req, "get_complaint", complaint_id=item.complaintID) for item in sql.complaintLink]
        return model
...
class ObservationDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Observation, req: Request) -> Self:
        ...
        if sql.documentationLink:
            model.documentation = DocumentationLinkDTO.from_list(sql.documentationLink, req)
        if sql.complaintLink:
            model.complaints = [get_url(req, "get_complaint", complaint_id=complaint.complaintID) for complaint in sql.complaintLink]
        return model
...
class ComplaintDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: Complaint, req: Request) -> Self:
        ...
        if sql.homologationLink:
            model.homologation = get_url(req, "get_homologation", homo_id=sql.homologationLink.homoID)
        if sql.agreementLink:
            model.agreement = get_url(req, "get_agreement", homo_id=sql.agreementLink.agreementID)
        if sql.observationLink:
            model.observation = get_url(req, "get_observation", obs_id=sql.observationLink.observationID)
        return model
...
class NonagreementInvoiceLinkDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: NonagreementInvoiceLink, req: Request) -> Self:
        ...
        model.invoice = get_url(req, "get_seclo_invoice", seclo_invoice_id=sql.secloInvoiceID)
        return model
...
class NonagreementSECLOInvoiceDTO(MyBaseModel):
    ...
    def from_sql(cls, sql: NonagreementSECLOInvoice, req: Request) -> Self:
        model = cls.model_validate(sql)
        model.nonagreements = NonagreementInvoiceLinkDTO.from_list(sql.nonagreementLink, req)
        return model
